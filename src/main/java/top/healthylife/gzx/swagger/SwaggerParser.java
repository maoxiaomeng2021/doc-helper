...
public class SwaggerParser {
    ...
    public Map<String, Object> parseBySwaggerJson(String jsonStr, String... apiPath) {
        Map<String, Object> resultMap = new HashMap<>();
        List<Table> result = new ArrayList<>();
        try {
            Map<String, Object> map = getResultFromString(result, jsonStr, apiPath);
    ...
    private Map<String, Object> getResultFromString(List<Table> result, String jsonStr, String... apiPath) throws IOException {
        // convert JSON string to Map
        Map<String, Object> map = JSONUtil.toBean(jsonStr, HashMap.class);

        //解析model
        Map<String, ModelAttr> definitinMap = parseDefinitions(map);

        //解析paths
        Map<String, Map<String, Object>> beforeFilterPaths = (Map<String, Map<String, Object>>) map.get("paths");
        //filter path
        Map<String, Map<String, Object>> paths = new HashMap<>();
        Arrays.stream(apiPath).forEach(e -> {
            if (beforeFilterPaths.keySet().contains(e)) {
                paths.put(e, beforeFilterPaths.get(e));
            }
        });
        ...
        if (paths != null) {
            ...
            while (it.hasNext()) {
                ...
                while (it2.hasNext()) {
                    ...
                    // 10.返回体
                    JSONObject responses = (JSONObject) content.get("responses");
     ...
     * @return
     */
    private List<Request> processRequestList(JSONArray parameters, Map<String, ModelAttr> definitinMap) {
        List<Request> requestList = new ArrayList<>();
        if (!CollectionUtils.isEmpty(parameters)) {

            for (Object paramObj : parameters) {
                JSONObject param = JSONUtil.parseObj(paramObj);
        ...
        if (!jsonMap.isEmpty()) {
            if (jsonMap.size() == 1) {
                for (Map.Entry<String, Object> entry : jsonMap.entrySet()) {
                    res += " -d '" + JSONUtil.toJsonStr(entry.getValue()) + "'";
                }
            } else {
                res += " -d '" + JSONUtil.toJsonStr(jsonMap) + "'";
            }
        }
        return res;
    }
    ...
    public static String buildHtmlStr(Map<String, Object> map, String template) {
        ...
        //DocBaseConfig.getConfigByKey("apiInfoTpl")
